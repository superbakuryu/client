...
def before_request():
    ...
    if 'user_name' in session:
        if mydb.partners.find_one({'email': session['user_name']}):
            ...
            g.services_registered = api.get_service_registered(session['id'])
            g.services_not_registered = api.get_services_not_registered(session['id'])
...
def login():
    if request.method == 'POST':
        ...
        if mydb.partners.find_one({'email': username}):
            print(3333333333)
...
def home():
    ...
    return render_template('index.html', user_name=session['user_name'], role=session['role'], id=session['id'],services_registered = g.services_registered, services_not_registered = g.services_not_registered)
...
@app.route('/service_info/<service_id>', methods=['GET', 'POST'])
def service_info(service_id):
    ...
    return render_template('service_info.html', user_name=session['user_name'], role=session['role'], service=find_service, id=session['id'],services_registered = g.services_registered, services_not_registered = g.services_not_registered)
